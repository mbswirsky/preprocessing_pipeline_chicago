...
def column_separator(df, target_col, beat_cols, date_cols, null_thresh=0.1, cat_thresh=100):
    """
    ...
    cat_cols = (
        ...
        .drop(columns=date_cols, errors='ignore')
        .describe(include=['O'])
        .loc[:, lambda x: x.loc['unique']<cat_thresh] # ignore columns with too many categories
        .columns
        .to_list()
    )
